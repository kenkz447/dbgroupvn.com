...
class ConstructionTableComponent extends React.Component<StateProps & DispatchProps> {
    selectedIds = []
    filters = {}
    componentWillReceiveProps(nextProps: StateProps) {
        if (nextProps.deleteResult)
            location.reload()
    ...
    getPagiOptions(): PaginationProps {
        if (!this.props.dataSource || !this.props.dataSource.entities)
            return null
    ...
    @autobind
    onSelectChange(selectedRowKeys, selectedRows: Array<ConstructionViewModel>) {
        if (!selectedRows)
            return
...
const mapStateToProps = (state: AdminRootState): StateProps => {
    return {
        dataSource: state.data.getIn(['CONSTRUCTION', 'response']),
        dataSourceProcessing: state.data.getIn(['CONSTRUCTION', 'status', 'processing']),
        deleteResult: state.data.getIn(['CONSTRUCTION_DELETE', 'response']),
        deleteProcessing: state.data.getIn(['CONSTRUCTION_DELETE', 'status', 'processing'])
    }
}
...
const mapDispatchToProps = (dispatch): DispatchProps => {
    const getDataSource = () => {
        ...
        const requestSendAction = RequestSend('CONSTRUCTION', {
            url: `/construction/getConstructions?${params.toString()}`
        })
    ...
    return {
        getDataSource,
        selectedDelete: (ids) => {
            const requestSendAction = RequestSend('CONSTRUCTION_DELETE', {
                url: `/construction/delete`,
                requestInit: {
                    method: 'DELETE',
                    headers: new Headers({
...
const ConstructionTableWithPureData = ExtractImmutableHOC(ConstructionTableComponent)
export const ConstructionTable = connect(mapStateToProps, mapDispatchToProps)(ConstructionTableWithPureData)